  ...
  return (
    <div className="min-h-screen flex">
      ...
      <div className="hidden lg:flex lg:w-1/2 relative overflow-hidden">
        <img src="https://images.unsplash.com/photo-1540575467063-178a50c2df87?w=1200" alt="Events" className="absolute inset-0 w-full h-full object-cover" />
        ...
        <div className="w-full max-w-md">
          ...
          <div className="bg-white/5 border border-white/10 rounded-xl p-4 mb-6">
            ...
            <div className="flex gap-2">
              <button onClick={() => fill('user')} className="flex-1 text-sm bg-primary/10 hover:bg-primary/20 text-primary border border-primary/30 py-2 px-3 rounded-lg transition-colors font-medium">
                👤 User
              </button>
              <button onClick={() => fill('admin')} className="flex-1 text-sm bg-purple-500/10 hover:bg-purple-500/20 text-purple-400 border border-purple-500/30 py-2 px-3 rounded-lg transition-colors font-medium">
                ⚙️ Admin
              </button>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} noValidate className="space-y-5">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className={`input pl-10 pr-11 ${errors.password ? 'border-red-500' : ''}`}
                />
                <button type="button" onClick={() => setShowPass(s => !s)} className="absolute right-3.5 top-1/2 -translate-y-1/2 text-gray-500 hover:text-gray-300 transition-colors">
            ...
            <button type="submit" disabled={loading} className="w-full btn-primary py-3.5 text-base flex items-center justify-center gap-2 shadow-lg shadow-primary/30 mt-2">
          ...
          <button className="w-full flex items-center justify-center gap-3 py-3.5 rounded-xl border border-white/10 text-gray-300 hover:text-white hover:border-white/25 hover:bg-white/5 transition-all font-medium">
            <FcGoogle className="w-5 h-5" />Continue with Google
          </button>

          <p className="text-center text-gray-500 mt-8 text-sm">
            Don't have an account?{' '}
            <Link to="/signup" className="text-primary hover:text-primary-light font-semibold transition-colors">Create account</Link>
          </p>
        </div>
      </div>
    </div>
