...
const BookingConfirmation = () => {
  ...
  const downloadTicket = () => {
    ...
    if (!ticketEl) return;
    const w = window.open('', '', 'width=620,height=850');
    w.document.write(`<!DOCTYPE html><html><head><title>EncoreHub Ticket</title>
      <style>
        body{background:#0a0a0a;color:#fff;font-family:Arial,sans-serif;padding:24px;margin:0}
        .ref{color:#E50914;font-size:22px;font-weight:900;letter-spacing:2px;font-family:monospace}
        .badge{display:inline-block;padding:4px 12px;border-radius:9999px;font-size:12px;font-weight:600}
        .confirmed{background:rgba(34,197,94,.15);color:#4ade80;border:1px solid rgba(34,197,94,.3)}
        .grid{display:grid;grid-template-columns:1fr 1fr;gap:16px;margin:16px 0}
        .label{color:#6b7280;font-size:11px;margin-bottom:4px}
        .value{color:#fff;font-size:13px;font-weight:600}
        .divider{border-top:2px dashed #2a2a2a;margin:16px 0}
        img{border-radius:8px;object-fit:cover}
        canvas{background:#fff;padding:8px;border-radius:8px}
      </style></head><body>${ticketEl.innerHTML}</body></html>`);
    w.document.close();
              ...
              <div className="mb-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {booking.seats?.map(seat => (
                    <span key={seat.seatNumber} className={`badge text-xs py-1.5 ${
                      seat.type === 'vip'     ? 'bg-yellow-500/20 text-yellow-400' :
                      seat.type === 'premium' ? 'bg-blue-500/20 text-blue-400' :
          ...
          <div className="bg-dark-card border border-dark-border rounded-b-2xl p-6 flex items-center justify-between">
            ...
            <div className="text-center">
              <div className="bg-white p-3 rounded-xl inline-block">
                <QRCanvas value={qrValue} size={100} />
              </div>
