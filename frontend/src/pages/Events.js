...
const Events = () => {
  ...
  // Raw search input (updates instantly as user types)
  ...
  const clearFilters = () => {
    setSearchInput('');
    setFilters({ category: '', city: '', minPrice: '', maxPrice: '', date: '' });
    setSearchParams({});
    setPage(1);
  };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-6">
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-3 rounded-lg border font-medium transition-all ${
              showFilters
                ? 'bg-primary border-primary text-white'
                : 'border-dark-border text-gray-300 hover:border-primary hover:text-primary bg-dark-card'
            }`}
          >
          ...
          <div className="flex items-center gap-1 bg-dark-card border border-dark-border rounded-lg p-1">
            <button
              ...
              className={`p-2 rounded ${viewMode === 'grid' ? 'bg-primary text-white' : 'text-gray-400 hover:text-white'}`}
            >
        ...
        {activeFiltersCount > 0 && (
          <div className="flex flex-wrap gap-2 mb-6">
            {debouncedSearch && (
              <span className="flex items-center gap-1.5 badge bg-primary/15 text-primary border border-primary/30 text-sm py-1.5">
                "{debouncedSearch}"
                <button onClick={() => setSearchInput('')} className="hover:text-white">
            ...
            {Object.entries(filters).filter(([, v]) => v).map(([key, value]) => (
              <span
                key={key}
                className="flex items-center gap-1.5 badge bg-primary/15 text-primary border border-primary/30 text-sm py-1.5"
              >
                {value}
            ...
            {pages > 1 && (
              <div className="flex justify-center gap-2 mt-10 flex-wrap">
                <button
                  ...
                  disabled={page === 1}
                  className="px-4 py-2 rounded-lg bg-dark-card border border-dark-border text-gray-300 hover:border-primary disabled:opacity-40 disabled:cursor-not-allowed transition-all"
                >
                  Prev
                </button>
                {[...Array(pages)].map((_, i) => (
